...
// ─────────────────────────────────────────────────────────────────────────────
// REUSABLE EMAIL SERVICE — HabitClick
...
class EmailService {
  ...
  async sendEmail({ to, subject, html }: SendEmailOptions) {
    try {
      const info = await this.transporter.sendMail({
        from: `"HabitClick" <${process.env.SMTP_USER || "no-reply@HabitClick.com"}>`,
        to,
        subject,
        html,
      })
  ...
  async sendOtpEmail(to: string, code: string, name?: string) {
    ...
    `
    return this.sendEmail({ to, subject: `${code} is your HabitClick verification code`, html })
  }
  ...
  async sendResetPasswordEmail(to: string, resetLink: string) {
    const html = `
      <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; line-height: 1.6;">
        ...
        <p>Click the button below to set a new password for your HabitClick account.</p>
    ...
    `
    return this.sendEmail({ to, subject: `Reset your HabitClick password`, html })
  }
}
